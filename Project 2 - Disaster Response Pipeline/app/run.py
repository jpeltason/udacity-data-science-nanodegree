...
def index():
    ...
    df_categories = pd.DataFrame(df.iloc[:, 4:].sum())
    ...
    df_category_genre = df.iloc[:, 3:].groupby(by=['genre']).sum().T
    fig = px.bar(df_category_genre, x=df_category_genre.index, 
            y=['direct', 'news', 'social'],
    ...
    fig = px.histogram(df_cat_per_message, x='Number of categories', 
    ...
    fig = px.bar(df_genres, x=df_genres.index, y='id', 
            labels={'x':'Message Genre', 'index': 'Number of Messages'},
            color_discrete_sequence=px.colors.qualitative.Bold,
